...
export async function groupInviteEmail(request: HttpRequest, _context: InvocationContext): Promise<HttpResponseInit> {
  ...
  const plainText = `${greeting}\n\n${inviterName} (${inviterEmail}) invited you to join the group "${group.groupName}" on Yapper.${personalText}\nJoin with this link:\n${inviteUrl}\n\nIf the link does not open, copy and paste it into your browser.\n\nYou are receiving this email because someone invited this address to a Yapper group. If this was unexpected, you can ignore it.\n\nNeed help? Contact support: support@yapper-app.com`;
  const html = `<div style="font-family:Arial,Helvetica,sans-serif;color:#111;line-height:1.5"><p style="margin:0 0 12px">${htmlEscape(greeting)}</p><p style="margin:0 0 12px"><strong>${htmlEscape(inviterName)}</strong> (${htmlEscape(inviterEmail)}) invited you to join the group <strong>${htmlEscape(group.groupName)}</strong> on Yapper.</p>${personalHtml}<p style="margin:0 0 16px"><a href="${inviteUrl}" style="display:inline-block;background:#1f6feb;color:#fff;text-decoration:none;padding:10px 14px;border-radius:6px;font-weight:600">Join group</a></p><p style="margin:0 0 8px;font-size:13px;color:#444">If the button does not work, use this link:</p><p style="margin:0 0 14px;word-break:break-all;font-size:13px"><a href="${inviteUrl}">${inviteUrl}</a></p><p style="margin:0 0 8px;font-size:12px;color:#666">You are receiving this email because someone invited this address to a Yapper group. If this was unexpected, you can ignore this message.</p><p style="margin:0;font-size:12px;color:#666">Need help? support@yapper-app.com</p></div>`;
