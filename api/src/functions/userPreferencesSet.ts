...
type UserPreferenceSetBody = { emailUpdatesEnabled?: unknown };
...
export async function userPreferencesSet(request: HttpRequest, _context: InvocationContext): Promise<HttpResponseInit> {
  ...
  const body = await request.json() as UserPreferenceSetBody;
  if (typeof body.emailUpdatesEnabled !== 'boolean') return json(400, { ok: false, message: 'emailUpdatesEnabled must be a boolean.' });

  const storage = createStorageAdapter();
  const prefs = await setEmailUpdatesEnabled(storage, email, body.emailUpdatesEnabled);
