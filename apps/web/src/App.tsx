        ...
        {snapshot.appointments.length === 0 ? <p>No appointments yet.</p> : (
          <ul>
            {snapshot.appointments.map((appointment) => (
              <li key={appointment.code}>{appointment.code} — {appointment.title}</li>
            ))}
          </ul>
        )}
      </section>
          ...
          <ul>
            {snapshot.availability.map((availability) => (
              <li key={availability.code}>
                {availability.code} — {availability.start.replace('T', ' ').slice(0, 16)}–{availability.end.slice(11, 16)}{availability.reason ? ` (${availability.reason})` : ''}
              </li>
            ))}
          </ul>
        )}
