...
export async function groupClaim(request: HttpRequest, _context: InvocationContext): Promise<HttpResponseInit> {
  ...
  const emailVerified = resolveEmailVerifiedFromSessionKind(graceSession.kind);
  if (!membership.ok) {
    ...
    try {
      ...
      logMembershipActivation({ traceId, groupId, email: normalizedEmail, memberKind, emailVerified, sessionKind: graceSession.kind, operation: 'create membership' });
    } catch (error) {
      logMembershipActivation({ traceId, groupId, email: normalizedEmail, memberKind, emailVerified, sessionKind: graceSession.kind, operation: 'create membership', error });
      throw error;
    }
    ...
    try {
      ...
      logMembershipActivation({ traceId, groupId, email: normalizedEmail, memberKind, emailVerified, sessionKind: graceSession.kind, operation: 'activate membership' });
    } catch (error) {
      logMembershipActivation({ traceId, groupId, email: normalizedEmail, memberKind, emailVerified, sessionKind: graceSession.kind, operation: 'activate membership', error });
      throw error;
    }
  }
